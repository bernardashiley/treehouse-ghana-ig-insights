...
recommendation surfaces, so they can be a useful lever for reaching new audiences.
Whether they outperform feed posts for a given account can only be judged where
that account has enough owned reels in the dataset; with very few reels the
comparison is exploratory. Feed posts (images and carousels) are seen mostly by
existing followers. \\[4pt]
...
owned-account average (the mean across the account's own deduplicated posts). A
lift of +100\% for a pillar means that, on average, a post in that pillar
attracted twice the engagement of a typical owned post. \\[4pt]
